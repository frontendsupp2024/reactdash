...
const StatCard = ({ data }: Props) => {
    return data.map((item, idx) => {
        return (
            <div className={`flex flex-col justify-center h-60`} key={idx}>
                {/* Top Heading */}
                <div style={{ backgroundColor: item.color }} className={`p-4 rounded-t-3xl text-center`}>
                    <h2 className='text-xl font-extrabold text-gray-800'>{item.title}</h2>
                </div>
                {/* Stats */}
                <div style={{ backgroundColor: item.colorLight }} className={`p-8 gap-4 flex items-center rounded-b-3xl`}>
